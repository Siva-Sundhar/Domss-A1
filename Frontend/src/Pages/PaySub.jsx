...
const PaySub = ({data, setData, inputRef, focusedIndex, isOpen}) => {
	const [forexCust] = useState(false)
				...
				<table>
					<thead>
						<tr className="leading-5 text-[13px] font-semibold border-y border-slate-400">
							...
							<td className=" align-top text-center w-36">Forex Currency Type</td>
									...
									<td className="text-center"> 
										<input
											...
											onKeyDown={(e) => {
												...
												if (e.key === "Enter") {
													isOpen(false);
													inputRef[(focusedIndex + 1) * 2]?.focus();
												}
											}}
										/>
									</td>
	...
	const suggestion = (index, fields)=>{
		...
		return(
			<div className="absolute -top-7 left-[100px] border border-slate-500 z-20 h-[105px] w-32 bg-[#def1fc]">
				...
				<ul onMouseDown={(e) => e.preventDefault()} tabIndex="-1" className="mt-3">
					{references.map((item, ind) => (
						<li 
						key={ind} 
						tabIndex="0"
						className={`px-2 h-[17px] flex justify-between items-center py-0.5  cursor-pointer text-sm ${selectIndex === ind ? "bg-amber-300" : ""}`}
				...
				<div className=" min-w-[465] flex justify-start items-center h-[100%] ">
					<div className="min-h-[480px] bg-white border border-slate-500 ">
                        ...
                        <div className="mb-2 flex justify-end leading-4">
                            <span className="text-right pr-1 font-bold border-t border-double text-[13px] border-b-4 border-slate-500 ">{NumberFormat(data[focusedIndex].amount)} Dr</span>
                        </div>
